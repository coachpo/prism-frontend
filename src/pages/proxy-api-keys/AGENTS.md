...
## OVERVIEW
`pages/proxy-api-keys/` owns global proxy-key issuance, rotation, deletion, auth-status messaging, and one-time secret display behind `../ProxyApiKeysPage.tsx`.
...
└── useProxyApiKeysPageData.ts    # Parallel bootstrap, create, rotate, delete, and badge state
```
...
- Patch the local key list after create, rotate, and delete flows instead of reloading the whole page.
...
- Do not reload the page after create, rotate, or delete when `useProxyApiKeysPageData.ts` already patches state locally.
