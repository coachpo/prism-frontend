...
- `useTimezone.ts`: Consumes timezone settings and provides locale-aware timestamp formatting through `src/i18n/format.ts`.
...
- Route shared date/time display through `useTimezone.ts` or `src/i18n/format.ts` rather than introducing route-local formatting helpers.
